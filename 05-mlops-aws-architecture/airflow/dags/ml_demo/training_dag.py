...
def mydag():
    ...
    def check_registered_model(data_files, reg_model, new_model_uri):
        ...
        _, new_val_f1 = get_val_performance(y_test, new_y_pred) # new_val_accuracy
        _, old_val_f1 = get_val_performance(y_test, old_y_pred) # old_val_accuracy

        logger.info(f'Old Model: {old_val_f1}')
        logger.info(f'New Model: {new_val_f1}')

        if new_val_f1 >= old_val_f1:
            logger.info(f"Registraremos una nueva version del modelo {reg_model}.")
